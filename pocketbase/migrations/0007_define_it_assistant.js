// pocketbase/migrations/0007_define_it_assistant.js
// Assistente IT — agente ÚNICO Skip Cloud (Fase 4 do Manual/Assistente IT).
// Base de conhecimento: Manual de Uso da Calculadora Markup (10 capítulos de src/content/manual/).
// Runtime: $ai.agent('it-assistant').chat(...) via pocketbase/hooks/it_assistant_ask.js (contexto de rota por mensagem).
migrate(
  (app) => {
    $ai.agents.define(app, {
      ...
      name: 'Assistente IT',
      description:
        'Guia oficial da plataforma IT: conduz o usuário no uso da Calculadora Markup e dos módulos, com o Manual de Uso como base de conhecimento.',
      systemPrompt: `Você é o Assistente IT, guia oficial da plataforma IT — Inteligência Tributária. Sua base de conhecimento é o Manual de Uso da Calculadora Markup (10 capítulos), fornecido a você como contexto RAG a cada mensagem.
...
6. Posicionamento: a plataforma IT não substitui o trabalho do profissional contábil — ela amplifica seus resultados. É uma ferramenta de apoio para gerar informações confiáveis com visão mais ampla e estratégica.
7. Dúvidas sobre Reforma Tributária (CBS, IBS, split payment): aponte as pílulas "Reforma Tributária" e "Ponte 2027" e use o Glossário (Capítulo 10).`,
      tier: 'fast',
      memory: [
        {
          type: 'text',
...
**Como evitar**: o sistema usa os três em lugares diferentes — "Margem de entrada" e "Margem líquida apurada" no modal de modos, "Margem de lucro (%)" no produto. Nomeie sempre qual está lendo.`,
  ...
  (app) => {
    $ai.agents.delete(app, 'it-assistant')
  },
)
