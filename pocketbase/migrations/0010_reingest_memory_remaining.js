// pocketbase/migrations/0010_reingest_memory_remaining.js
// Correção da ingestão RAG: as 10 fontes originais (17:02) ficaram presas em "processing" com
// 0 chunks (searchMemory vazio — diagnosticado com hook zz-diag-memory). A fonte [v2] do cap. 6
// (migration 0009) indexou em segundos e o agente passou a citar o manual. Esta migration
// re-ingere os 9 capítulos restantes com o mesmo padrão [v2] (payload_key nova → novo ciclo).
// As 10 fontes originais presas permanecem no banco, inertes (0 chunks) — limpeza opcional depois.
migrate(
  (app) => {
    $ai.agents.putMemories(app, 'it-assistant', [
      {
        ...
        payload: {
          text: `[v2]
# Capítulo 1 — Primeiros passos e o fluxo ideal
...
**Como evitar**: são **camadas separadas** — a origem diz quem manda no número. Para mudar o custo de origem Compras, mude na Compras e puxe de novo (o padrão idempotente cuida do resto).`,
      ...
      },
      {
        ...
        payload: {
          text: `[v2]
# Capítulo 10 — Glossário tributário
...
**Como evitar**: o sistema usa os três em lugares diferentes — "Margem de entrada" e "Margem líquida apurada" no modal de modos, "Margem de lucro (%)" no produto. Nomeie sempre qual está lendo.`,
  ...
  (app) => {
    // Down: remove as 9 fontes [v2] adicionadas aqui (conteúdo exato exigido pela chave).
    // O cap. 6 [v2] (migration 0009) tem down próprio.
  },
)
