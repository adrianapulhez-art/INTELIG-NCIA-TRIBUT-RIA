 ...
 * AGUARDANDO CHANCELA DA CEO (substituem os ouros v1 de 22/09).
 * ============================================================================
 */
...
describe('CMV Art. 12 v2 — invariante central (cadeia plena neutra)', () => {
  ...
  it('fator de repasse v2: (1−ICMS)×(1−PIS/COFINS)÷(1−ICMS×fração) — 2027 LP = 0,963500', () => {
    ...
    // (0,82 × 0,9635) ÷ (1 − 0,108) = 0,79007 ÷ 0,892
    expect(f2032.toFixed(6)).toBe('0.885729')
  })

  it('HOJE intacto: régua canônica R$ 1.158,93/un (LP) e R$ 1.051,73/un (LR)', () => {
    const row2027 = CRONOGRAMA_ART12.find((r) => r.exercicio === 2027)!
    const lp = computeCellArt12(CASO_CANONICO_ART12, { ...CONFIG_PADRAO_ART12 }, row2027)
    expect(fmt(lp.hoje.unitario)).toBe('1158.93')
    expect(fmt(lp.hoje.bruto)).toBe('42400.00')
    expect(fmt(lp.hoje.creditos)).toBe('7632.00')
    ...
    // crédito PIS/COFINS do LR sobre BASE SEM ICMS (tese do século)
    expect(fmt(lr.hoje.lines.find((l) => l.key === 'piscofins')!.value)).toBe('-3216.04')
  })
  ...
  it('2026: sem reprecificação (f=1) e custo idêntico ao HOJE nas cadeias plenas (CBS teste compensável)', () => {
    ...
    const pis = lp.exercicio.lines.find((l) => l.key === 'piscofins')!
    expect(fmt(pis.value)).toBe('-1269.03')
    expect(pis.fundamento.dispositivo).toContain('§2º, V')
    const lr = computeCellArt12(
      CASO_CANONICO_ART12,
      { ...CONFIG_PADRAO_ART12, fornecedorRegime: 'real', compradorRegime: 'real' },
      row,
...
describe('CMV Art. 12 v2 — estrutura em 2 blocos (proposta da CEO)', () => {
  ...
  it('bloco 1 — formação do preço do fornecedor: 42.000×0,9635 + frete − ICMS → base limpa 33.498,97 → +CBS 2.947,91 +IBS 33,50 → nota 44.488,27 (tese do Fisco)', () => {
    const b1 = cell.exercicio.lines.filter((l) => l.bloco === 1)
    const get = (k: string) => b1.find((l) => l.key === k)!
    expect(fmt(get('mercadoria').value)).toBe('40467.00')
    expect(fmt(get('frete').value)).toBe('385.40')
    expect(fmt(get('icms').value)).toBe('-7353.43')
...
describe('CMV Art. 12 v2 — réguas de repasse (quem absorve)', () => {
  it('ouro 2027 (iguais nas duas teses): integral 1.116,63 (−3,65%) · parcial 1.137,78 (−1,82%) · nenhum 1.158,93 (0,00%)', () => {
    ...
    for (const tese of ['fisco', 'contribuinte'] as const) {
      ...
      expect(fmt(byMode.integral.cell.exercicio.unitario)).toBe('1116.63')
      expect(fmt(byMode.integral.cell.deltaPct)).toBe('-3.65')
      expect(fmt(byMode.parcial.cell.exercicio.unitario)).toBe('1137.78')
      expect(fmt(byMode.parcial.cell.deltaPct)).toBe('-1.82')
  ...
  it('escada régua NENHUM inalterada: 1.158,93 → 1.260,69 (2027→2032), degrau constante +25,44/un', () => {
    ...
    for (let i = 2; i < esc.length; i++) {
      const degrau = r2(esc[i].unitario - esc[i - 1].unitario)
      expect(fmt(degrau)).toBe('25.44')
    }
  })

  it('escada 2026 (novo ouro): 1.116,63 — a queda do PIS/COFINS embutido chega ao comprador mesmo sem repasse', () => {
    const esc = escadaArt12(CASO_CANONICO_ART12, { ...CONFIG_PADRAO_ART12 })
    expect(fmt(esc[0].unitario)).toBe('1116.63')
    expect(fmt(esc[0].deltaPct)).toBe('-3.65')
  })
})
...
describe('CMV Art. 12 v2 — matriz 3×3 (2027)', () => {
  ...
  it('ouro por célula — tese do FISCO (unitário/delta vs próprio HOJE)', () => {
    ...
    // Comprador SN: paga a nota inteira — tese do Fisco: 44.488,27 → 1.482,94
    ...
    expect(fmt(get('presumido', 'simples').exercicio.unitario)).toBe('1413.33')
  })
  ...
  it('ouro por célula — tese do CONTRIBUINTE: pleno idêntico; SN muda (nota menor)', () => {
    ...
    expect(fmt(get('presumido', 'presumido').exercicio.unitario)).toBe('1116.63')
    expect(fmt(get('real', 'real').exercicio.unitario)).toBe('1051.73')
    // Comprador SN paga operação + CBS + IBS por fora: 43.833,81 → 1.461,13
...
describe('CMV Art. 12 v2 — indústria e ZFM (IPI §2º, II)', () => {
  ...
  it('indústria 2026: IPI 10% na nota, crédito integral ao industrial — custo = HOJE (1.158,93)', () => {
    ...
    expect(fmt(cell.exercicio.lines.find((l) => l.key === 'piscofins')!.value)).toBe('-1269.03')
  })
})
...
describe('CMV Art. 12 v2 — camada de auditoria (cada passo recalculado = valor exibido)', () => {
  ...
  it('Mercadoria 2027: os passos reproduzem o fator 0,963500 e o valor exibido (6 casas e centavo)', () => {
    const row = CRONOGRAMA_ART12.find((r) => r.exercicio === 2027)!
    ...
    expect(passos[5].resultado).toBe('0,963500')
    ...
    expect(fmt(linha.value)).toBe('40467.00')
  })
  ...
  it('Compras líquidas 2027: nota − créditos = 33.498,97 (identidade ao centavo)', () => {
    const row = CRONOGRAMA_ART12.find((r) => r.exercicio === 2027)!
    const cell = computeCellArt12(CASO_CANONICO_ART12, { ...CONFIG_PADRAO_ART12 }, row)
    const cl = cell.exercicio.lines.find((l) => l.key === 'comprasliquidas')!
    const recalc = r2(
      cell.exercicio.bruto -
        -cell.exercicio.lines.find((l) => l.key === 'creditoicms')!.value -
        -cell.exercicio.lines.find((l) => l.key === 'creditocbs')!.value -
        -cell.exercicio.lines.find((l) => l.key === 'creditoibs')!.value,
    )
    expect(fmt(cl.value)).toBe(fmt(recalc))
    expect(fmt(cl.value)).toBe('33498.97')
  })

  it('fórmula exibida da linha nunca arredonda o fator (contém 0,963500, não 0,96)', () => {
    const row = CRONOGRAMA_ART12.find((r) => r.exercicio === 2027)!
    const cell = computeCellArt12(CASO_CANONICO_ART12, { ...CONFIG_PADRAO_ART12 }, row)
    const linha = cell.exercicio.lines.find((l) => l.key === 'mercadoria')!
    expect(linha.formula).toContain('0,963500')
